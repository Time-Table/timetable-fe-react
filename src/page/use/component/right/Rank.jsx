...
// 순위에 따른 그라데이션을 반환하는 헬퍼 함수
const getRankGradient = (ranking) => {
     if (ranking === 1) {
          // 1등: 하얀 빛이 도는 부드러운 옐로우 그라데이션
          return "linear-gradient(135deg, #fffeeb, #ffb300)";
     }
     // 2등 이하: 사이트의 primary 색상 기반 그라데이션
...
export default function Rank({ setRightScreen, timeInfo = [], selectedName, setSelectedName }) {
     ...
     // 수정된 순위 계산 로직 (Dense Rank: 1, 1, 2, 3...)
     ...
     // 최대/최소 인원 수를 계산하여 불투명도 범위를 정하는 데 사용
